...
/// FUNÇÕES DE RELATÓRIOS DE DADOS DE DISCIPLINAS
...
void listarProfessores() {
  ...
  enum MENU {
    ...
    Filtro_Nome = '3',
    Filtro_Nascimento = '4',
    Filtro_por_tamanho_de_grade = '5',
    Filtro_por_palavra_chave = '6',
    Voltar = '0'
  };
  ...
  if (professoresCadastrados > 0) {
    ...
    puts("\t> Não há professores cadastrados <\n");
  }
}
void listarAlunos() {
  int menuListarAluno();
...
void listarDisciplinas() {
  ...
  enum MENU {
    Filtro_Geral = '1',
    Filtro_disciplinas_com_alunos = '2',
    Filtro_disciplinas_vagas = '3',
    Voltar = '0'
  };
...
void ordernaProfessoresPorSexo(Professor *professores) {
  ...
  for (int ordenador = professoresCadastrados - 1; ordenador >= 1;
       ordenador--) {
...
void ordernaProfessoresPorOrdemAlfabetica(Professor *professores) {
  ...
  for (int ordenador = professoresCadastrados - 1; ordenador >= 1;
       ordenador--) {
    for (int i = 0; i < ordenador; i++) {
      if ((strcmp(professores[i].dado.nome, professores[i + 1].dado.nome)) >
          0) {
...
void ordernaProfessoresPorNascimento(Professor *professores) {
  Professor aux;
  char AAMM_1[6], AAAA_1[5], AAMM_2[6], AAAA_2[5];
  ...
  for (int ordenador = professoresCadastrados - 1; ordenador >= 1;
    ...
    for (int i = 0; i < ordenador; i++) {
      ...
      if (strcmp(AAAA_1, AAAA_2) > 0) {
        ...
        strcpy(professores[i].dado.nascimento,
               professores[i + 1].dado.nascimento);
    ...
    if (sair(mesAtual[0], mesAtual[1])) {
      ...
      if (mesAtualNum > 12 || mesAtualNum < 1) {
        ...
        if (aniversariantesDoMes[0] != -1) {
          ...
          for (int corredor = 0; aniversariantesDoMes[corredor] != -1;
               corredor++) {
            printf("%*s %*s \n", TAM_NAC + 11,
                   discente[aniversariantesDoMes[corredor]].dado.nascimento, 10,
                   discente[aniversariantesDoMes[corredor]].dado.nome);
          }
        } else {
        ...
        if (aniversariantesDoMes[0] != -1) {
          ...
          for (int corredor = 0; aniversariantesDoMes[corredor] != -1;
               corredor++) {
            printf("%*s %*s \n", TAM_NAC + 11,
                   docente[aniversariantesDoMes[corredor]].dado.nascimento, 10,
                   docente[aniversariantesDoMes[corredor]].dado.nome);
          }
        } else {
...
int *alunosAniversariantes(char *mesAtual) {
  Aluno aniversarianteDoMes[alunosCadastrados], alunoBackup;
  int quantidade = 0;
  char mes[3], dia_1[3], dia_2[3];

  for (int i = 0; i < alunosCadastrados; i++) {
    aniversarianteDoMes[i] = discente[i];
    if (mesAtual[0] == discente[i].dado.nascimento[3] &&
        mesAtual[1] == discente[i].dado.nascimento[4]) {
      quantidade++;
    }
  }
  quantidade++;
  ...
  for (int x = 1; x < alunosCadastrados; x++) {
    for (int i = 0; i < alunosCadastrados - 1; i++) {
      ...
      if (strcmp(dia_1, dia_2) > 0) {
        strcpy(alunoBackup.dado.nascimento,
               aniversarianteDoMes[i].dado.nascimento);
        strcpy(aniversarianteDoMes[i].dado.nascimento,
               aniversarianteDoMes[i + 1].dado.nascimento);
        strcpy(aniversarianteDoMes[i + 1].dado.nascimento,
               alunoBackup.dado.nascimento);
      }
    }
  }
  ...
  for (int i = 0; i < alunosCadastrados; i++) {
    ...
    if ((strcmp(mesAtual, mes)) == 0) {
      for (int y = 0; y < alunosCadastrados; y++) {
        if (strcmp(aniversarianteDoMes[i].dado.nascimento,
                   discente[y].dado.nascimento) == 0) {
...
int *professoresAniversariantes(char *mesAtual) {
  Professor aniversarianteDoMes[professoresCadastrados], professoresBackup;
  int quantidade = 0;
  char mes[3], dia_1[3], dia_2[3];

  for (int i = 0; i < professoresCadastrados; i++) {
    aniversarianteDoMes[i] = docente[i];
    if (mesAtual[0] == docente[i].dado.nascimento[3] &&
        mesAtual[1] == docente[i].dado.nascimento[4]) {
      quantidade++;
    }
  }
  quantidade++;
  ...
  for (int x = 1; x < professoresCadastrados; x++) {
    for (int i = 0; i < professoresCadastrados - 1; i++) {
      ...
      dia_2[2] = '\0';
      if (strcmp(dia_1, dia_2) > 0) {
        strcpy(professoresBackup.dado.nascimento,
               aniversarianteDoMes[i].dado.nascimento);
        strcpy(aniversarianteDoMes[i].dado.nascimento,
               aniversarianteDoMes[i + 1].dado.nascimento);
        strcpy(aniversarianteDoMes[i + 1].dado.nascimento,
               professoresBackup.dado.nascimento);
      }
    }
  }
...
void relatorioAlunos(Aluno alunosOrdenados[]) {
  printf(
      ...
      "====================================================================="
      "====================================================================\n");
  printf(" %*s  %*s  %*s  %*s  %*s  \n", TAM_MAT + 4, "MATRÍCULA", TAM_CPF,
         "CPF", TAM_NAC + 11, "NASCIMENTO", 14, "SEXO", 14, "NOME");
  for (int posicao = 0; posicao < alunosCadastrados; posicao++) {
    printf("%*s  %*s  %*s      %*c      %-*s\n", TAM_MAT + 5,
           alunosOrdenados[posicao].dado.matricula, TAM_CPF + 5,
           alunosOrdenados[posicao].dado.cpf, TAM_NAC + 5,
           alunosOrdenados[posicao].dado.nascimento, 9,
           alunosOrdenados[posicao].dado.sexo, TAM_NOME,
           alunosOrdenados[posicao].dado.nome);
  }
  puts("\n");
}
void relatorioProfessores(Professor professoresOrdenados[]) {
  printf(
      ...
      "====================================================================="
      "====================================================================\n");
  printf(" %*s  %*s  %*s  %*s  %*s  \n", TAM_MAT + 5, "MATRÍCULA", TAM_CPF,
         "CPF", TAM_NAC + 11, "NASCIMENTO", 12, "SEXO ", 14, "NOME");
  for (int posicao = 0; posicao < professoresCadastrados; posicao++) {
    printf(" %*s  %*s  %*s  %*c          %-s\n", TAM_MAT + 5,
           professoresOrdenados[posicao].dado.matricula, TAM_CPF + 5,
           professoresOrdenados[posicao].dado.cpf, TAM_NAC + 5,
           professoresOrdenados[posicao].dado.nascimento, 9,
           professoresOrdenados[posicao].dado.sexo,
...
void relatorioDisciplinas() {
  printf(
      ...
      "======================================================================"
      "===================================================================\n");
  printf("%*s %*s %*s %*s %*s\n", TAM_COD_DISC + 4, "CÓDIGO", TAM_SEM + 4,
         "SEMESTRE", 11, "RELAÇÃO", 18, "PROFESSOR", 18, "NOME");
  for (int posicao = 0; posicao < disciplinasCadastradas; posicao++) {
    printf("%*s  %*s  %*d/%2.d      %-*s  %-*s\n", TAM_COD_DISC + 3,
           materia[posicao].codigo, TAM_SEM + 2, materia[posicao].semestre, 4,
           materia[posicao].quantidadeMatriculado, materia[posicao].vagas, 20,
           docente[materia[posicao].professorId - 1].dado.nome, 20,
...
void disciplinaComAlunos(int posicao) {
  ...
  for (int matriculado = 0; matriculado < alunosCadastrados; matriculado++) {
    for (int cursando = 0; cursando < discente[matriculado].disciplinasCursando;
         cursando++) {
      if (discente[matriculado].matrizCurricular[cursando] == posicao) {
        contadorDeAlunos++;
        printf(" %*s      %-*s\n", TAM_MAT + 2,
               discente[matriculado].dado.matricula, TAM_NOME + 6,
               discente[matriculado].dado.nome);
      }
    }
  }
    ...
    if (vagas == -1) {
      ...
      if (vagas < 0) {
        ...
        printf("==============================================================="
               ...
               "==============================================================="
               "========================\n");
        printf("%*s %*s %*s %*s %*s\n", TAM_COD_DISC + 4, "CODIGO", TAM_SEM + 4,
               "SEMESTRE", 11, "VAGAS", 18, "PROFESSOR", 18, "NOME");
        for (int posicao = 0; posicao < disciplinasCadastradas; posicao++) {
          if (materia[posicao].vagas > vagas) {
            printf("%*s  %*s  %*d      %-*s     %-*s\n", TAM_COD_DISC + 3,
                   materia[posicao].codigo, TAM_SEM + 2,
                   materia[posicao].semestre, 9, materia[posicao].vagas, 20,
                   docente[materia[posicao].professorId - 1].dado.nome, 20,
                   materia[posicao].nome);
          }
        }
    ...
    if (matriculado == -1) {
      ...
      if (matriculado < 0) {
        ...
        printf("==============================================================="
               ...
               "=====\n");
        printf(" %*s  %*s  %*s  %*s  %*s  \n", TAM_MAT + 4, "MATRÍCULA",
               TAM_CPF, "CPF", TAM_NAC + 11, "NASCIMENTO", 14, "SEXO", 14,
               "NOME");
        for (int posicao = 0; posicao < alunosCadastrados; posicao++) {
          if (discente[posicao].disciplinasCursando < matriculado) {
            printf("%*s  %*s  %*s  %*c          %-*s\n", TAM_MAT + 5,
                   discente[posicao].dado.matricula, TAM_CPF + 5,
                   discente[posicao].dado.cpf, TAM_NAC + 5,
                   discente[posicao].dado.nascimento, 9,
                   discente[posicao].dado.sexo, TAM_NOME,
                   discente[posicao].dado.nome);
          }
        }
      }
    }
  ...
  do {
    ...
    if (matriculado == -1) {
      ...
      if (matriculado < 0) {
        ...
        for (int posicao = 0; posicao < professoresCadastrados; posicao++) {
          if (docente[posicao].disciplinasMinistrando < matriculado) {
            printf("%*s  %*s  %*s     %*c        %-*s\n", TAM_MAT + 5,
                   docente[posicao].dado.matricula, TAM_CPF + 5,
                   docente[posicao].dado.cpf, TAM_NAC + 5,
                   docente[posicao].dado.nascimento, 9,
                   docente[posicao].dado.sexo, TAM_NOME,
                   docente[posicao].dado.nome);
          }
        }
      }
    }
...
int solicitaCodigoFiltro() {
  ...
  do {
    ...
    if (sair(codigo[0], codigo[1])) {
      ...
      if (DADO == VALIDO) {
        posicao = verificaCodigoDisciplina(codigo);
        if (posicao < 0) {
...
void filtroPorPalavraChave() {
  ...
  do {
    ...
    if (sair(palavraChave[0], palavraChave[1])) {
      ...
      if (validaNome(palavraChave)) {
        ...
        if (!(encontradosId[0] == -1)) {
          ...
          for (int posicao = 0; encontradosId[posicao] != -1; posicao++) {
            printf("%*s  %*s  %*s    %*c         %-*s\n", TAM_MAT + 5,
                   discente[encontradosId[posicao]].dado.matricula, TAM_CPF + 5,
                   discente[encontradosId[posicao]].dado.cpf, TAM_NAC + 5,
                   discente[encontradosId[posicao]].dado.nascimento, 9,
                   discente[encontradosId[posicao]].dado.sexo, TAM_NOME,
                   discente[encontradosId[posicao]].dado.nome);
          }
        } else {
        ...
        if (!(encontradosId[0] == -1)) {
          ...
          for (int posicao = 0; encontradosId[posicao] != -1; posicao++) {
            printf("%*s  %*s  %*s     %*c       %-*s\n", TAM_MAT + 5,
                   docente[encontradosId[posicao]].dado.matricula, TAM_CPF + 5,
                   docente[encontradosId[posicao]].dado.cpf, TAM_NAC + 5,
                   docente[encontradosId[posicao]].dado.nascimento, 9,
                   docente[encontradosId[posicao]].dado.sexo, TAM_NOME,
                   docente[encontradosId[posicao]].dado.nome);
          }
        } else {
      ...
      puts("\t> Digite no mínimo 3 letras < \n");
    }
  } while (true);
  puts("\n");
}
      ...
      if (validaNome(palavraChave)) {
        ...
        if (!(alunosEncontradosId[0] == -1)) {
          ...
          for (int posicao = 0; alunosEncontradosId[posicao] != -1; posicao++) {
            printf("%*s  %*s  %*s    %*c         %-*s\n", TAM_MAT + 5,
                   discente[alunosEncontradosId[posicao]].dado.matricula,
                   TAM_CPF + 5, discente[alunosEncontradosId[posicao]].dado.cpf,
                   TAM_NAC + 5,
                   discente[alunosEncontradosId[posicao]].dado.nascimento, 9,
                   discente[alunosEncontradosId[posicao]].dado.sexo, TAM_NOME,
                   discente[alunosEncontradosId[posicao]].dado.nome);
          }
        } else {
          puts("\n > Nenhum aluno foi encontrado < \n");
        }
...
int *dispositivoBuscadorDeAluno(char *chave) {
  int *alunosEncontradosId = (int *)malloc(alunosCadastrados * sizeof(int)),
      corredor = 0, tamanhoChave = strlen(chave);
  char *recorte = (char *)malloc(tamanhoChave * sizeof(char));

  if (alunosCadastrados > 0) {
    for (int posicao = 0; posicao < alunosCadastrados; posicao++) {
      for (int caracter = 0; discente[posicao].dado.nome[caracter] != '\0';
           caracter++) {
        if (discente[posicao].dado.nome[caracter] == chave[0] &&
            discente[posicao].dado.nome[caracter + tamanhoChave - 1] ==
                chave[tamanhoChave - 1]) {
          strncpy(recorte, &discente[posicao].dado.nome[caracter],
                  tamanhoChave);
...
void buscarProfessoresPorPalavraChave() {
  ...
  do {
    ...
    printf("\n================================\n"
           "||   Filtro - PALAVRA CHAVE   || \n"
           "================================\n"
           "\nPESQUISE PROFESSORES CADASTRADOS\n\n"
           "- Digite a palavra chave (min 3 letras): \n"
           "-> ");
    fgets(palavraChave, TAM_NOME, stdin);
    if (sair(palavraChave[0], palavraChave[1])) {
      ...
      if (validaNome(palavraChave)) {
        ...
        if (!(professoresEncontradosId[0] == -1)) {
          ...
          printf(" %*s  %*s  %*s  %*s  %*s  \n", TAM_MAT + 4, "MATRÍCULA",
                 TAM_CPF, "CPF", TAM_NAC + 11, "NASCIMENTO", 14, "SEXO", 14,
                 "NOME");
          for (int posicao = 0; professoresEncontradosId[posicao] != -1;
               posicao++) {
            printf("%*s  %*s  %*s     %*c       %-*s\n", TAM_MAT + 5,
                   docente[professoresEncontradosId[posicao]].dado.matricula,
                   TAM_CPF + 5,
                   docente[professoresEncontradosId[posicao]].dado.cpf,
                   TAM_NAC + 5,
                   docente[professoresEncontradosId[posicao]].dado.nascimento,
                   9, docente[professoresEncontradosId[posicao]].dado.sexo,
                   TAM_NOME,
                   docente[professoresEncontradosId[posicao]].dado.nome);
          }
        } else {
          puts("\n > Nenhum professor foi encontrado < \n");
        }
...
int *dispositivoBuscadorDeProfessor(char *chave) {
  int *professoresEncontradosId =
          (int *)malloc(professoresCadastrados * sizeof(int)),
      corredor = 0, /// incluir mensagem para falha na alocação de memória
      tamanhoChave = strlen(chave);
  char *recorte = (char *)malloc(tamanhoChave * sizeof(char));

  if (professoresCadastrados > 0) {
    for (int posicao = 0; posicao < professoresCadastrados; posicao++) {
      for (int caracter = 0; docente[posicao].dado.nome[caracter] != '\0';
           caracter++) {
        if (docente[posicao].dado.nome[caracter] == chave[0] &&
            docente[posicao].dado.nome[caracter + tamanhoChave - 1] ==
                chave[tamanhoChave - 1]) {
