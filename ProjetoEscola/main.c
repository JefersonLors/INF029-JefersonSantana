...
#include "./DataRecovers/DataRecovers.h"
...
int main() {
  ...
  if (criaArquivos() && recuperaDadosGerais() && recuperaCadastrosAlunos() &&
      recuperaCadastrosProfessores() && recuperaCadastrosDisciplinas()) {
    do {
      opcao = menuPrincipal();
      switch (opcao) {
      case Realizar_Cadastro:
        cadastrar();
        break;
